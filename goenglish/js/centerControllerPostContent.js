...
app.controller('myCtrl', function($sce, $scope, $http) {
	...
	$scope.takeData = function(){
		...
		let url = "http://golanguages.edu.vn:8000/api/posts/show/"+$scope.curType+"/"+$scope.curNum+"/"+$scope.curPage;
		$http.get(url)
	...
	$scope.postTaker = function(){
		...
		console.log($scope.curType, $scope.curNum, $scope.curPage);
		let url = "http://golanguages.edu.vn:8000/api/posts/show/content/"+/id=(\d*)/.exec(document.URL)[1];
		$http.get(url)
		.then(function(response) {
